  ...
  for (int _text = box_width; _text < text_len; _text++) {
    ...
    for (int _bit = 0; _bit < 8; _bit++) {
      for (int _pixel = 0; _pixel < text_box_pixel; _pixel++) {
        // ESP_LOGI(__FUNCTION__, "_text=%d _bit=%d _pixel=%d", _text, _bit,
        // _pixel);
...
void ssd1306_software_scroll(SSD1306_t *dev, int start, int end) {
  ESP_LOGD(__FUNCTION__, "software_scroll start=%d end=%d _pages=%d", start,
           end, dev->_pages);
  if (start < 0 || end < 0) {
    dev->_scEnable = false;
...
void ssd1306_scroll_text(SSD1306_t *dev, const char *text, int text_len,
  ...
  void (*func)(SSD1306_t *dev, int page, int seg, const uint8_t *images,
               int width);
  if (dev->_address == SPI_ADDRESS) {
  ...
  if (scroll == SCROLL_RIGHT) {
    ...
    for (int page = 0; page < dev->_pages - 1; page++) {
      ...
      for (int seg = _start; seg <= _end; seg++) {
        ...
        wk2 = wk0 | wk1;
        if (seg == 0) {
          ESP_LOGD(__FUNCTION__, "a page=%d wk0=%02x wk1=%02x wk2=%02x", page,
                   wk0, wk1, wk2);
        }
        if (dev->_flip)
...
void _ssd1306_bitmaps(SSD1306_t *dev, int xpos, int ypos, const uint8_t *bitmap,
  ...
  ESP_LOGD(__FUNCTION__, "_width=%d ypos=%d page=%d dstBits=%d", _width, ypos,
           page, dstBits);
  int offset = 0;
    ...
    for (int index = 0; index < _width; index++) {
      for (int srcBits = 7; srcBits >= 0; srcBits--) {
        ...
        ESP_LOGD(__FUNCTION__,
                 "index=%d offset=%d wk1=0x%x page=%d _seg=%d, wk2=%02x", index,
                 offset, wk1, page, _seg, wk2);
        if (_seg >= 128) {
...
void ssd1306_bitmaps(SSD1306_t *dev, int xpos, int ypos, const uint8_t *bitmap,
  ...
  for (int page = start_page; page <= end_page; page++) {
    ...
    ssd1306_display_image(dev, page, seg_start,
...
void _ssd1306_pixel(SSD1306_t *dev, int xpos, int ypos, bool invert) {
  ...
  uint8_t wk1 = 1 << _bits;
  ESP_LOGD(__FUNCTION__, "ypos=%d _page=%d _bits=%d wk0=0x%02x wk1=0x%02x",
           ypos, _page, _bits, wk0, wk1);
  if (invert) {
    wk0 = wk0 & ~wk1;
  } else {
    wk0 = wk0 | wk1;
...
uint8_t ssd1306_copy_bit(uint8_t src, int srcBits, uint8_t dst, int dstBits) {
  ESP_LOGD(__FUNCTION__, "src=%02x srcBits=%d dst=%02x dstBits=%d", src,
           srcBits, dst, dstBits);
...
void ssd1306_rotate_image(uint8_t *image, bool flip) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    for (int j = 0; j < 8; j++) {
      ...
      ESP_LOGD(__FUNCTION__, "image[%d]=0x%x _smask=0x%x _wk=0x%x", j, image[j],
               _smask, _wk);
      if (_wk != 0) {
...
void ssd1306_dump_page(SSD1306_t *dev, int page, int seg) {
  ESP_LOGI(__FUNCTION__, "dev->_page[%d]._segs[%d]=%02x", page, seg,
