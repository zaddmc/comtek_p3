...
void SHA256(unsigned char *data, char out_data[SHA256_OUT_LEN]) {
...
void RollingSHA256(char out_data[SHA256_OUT_LEN], char *data, size_t counter,
  ...
  unsigned char concated_data[data_len + digit_size + 1];
  snprintf((char *)concated_data, digit_size + data_len + 1, "%s%d", data,
           (char)counter);
  concated_data[data_len + digit_size] = 0;
  SHA256(concated_data, out_data);
}
