...
char *RollingSHA256(char *data, size_t counter) {
  ...
  unsigned char concated_data[data_len + MAX_DIGITS];
  snprintf((char *)concated_data, MAX_DIGITS + data_len, "%s%d", data,
           (char)counter);
  concated_data[data_len + MAX_DIGITS] = 0;
  printf("%s\n", concated_data);
  return SHA256(concated_data);
}
