...
int32_t nvs_read_counter() {
  ...
  ESP_LOGI(TAG, "SUCCESSFULLY READ COUNTER");
...
esp_err_t nvs_write_counter(int32_t new_counter) {
  ...
  ESP_LOGI(TAG, "SUCCESSFULLY WROTE TO COUNTER");
...
struct CustStr nvs_read_key() {
  ...
  ESP_LOGI(TAG, "SUCCESSFULLY READ SECRET KEY");
...
esp_err_t nvs_erase_main_keys() {
  ...
  ret = nvs_erase_key(nvs_handler, "secret_key");
